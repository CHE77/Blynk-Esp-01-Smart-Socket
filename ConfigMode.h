...
/// добавлены библиотеки реального времени
#include <TimeLib.h>
#include <WidgetRTC.h>
WidgetRTC rtc;
///
...
<div class="centered">
  <form method="get" action="config">
    ...
    <tr><td><label for="blynk">Auth token:</label></td><td><input type="text" name="blynk" placeholder="a0b1c2d... from Blynk app" pattern="[-_a-zA-Z0-9]{32}" maxlength="32" required="required"></td></tr>
    ...
    <tr><td><label for="device_name">Device: name:</label></td>       <td><input type="text" name="device_name" placeholder="update Device name if neccessary" length=64></td></tr>
    </table><br/>
...
void enterConfigMode()
{
  ...
  server.on("/config", []() {
    ...
    if (token.length() == 32 && ssid.length() > 0) {
      ...
      CopyString(device_name, configStore.clouddevice_name);
      }
...
void enterConnectNet() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    ///BlynkState::set(MODE_ERROR);
     DEBUG_PRINT(String("Not connected to WiFi: ") + configStore.wifiSSID);      
     BlynkState::set(MODE_CONFIGURING);////
     enterConfigMode();////
  }
}
